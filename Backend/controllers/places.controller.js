...
const getPlacesByUserId = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not find the places",
      500,
    );
    return next(error);
  }
...
const createPlace = async (req, res, next) => {
  ...
  try {
    ...
    await createdPlace.save({ session: currSession });
...
const updatePlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went error, could not update place.",
      500,
    );
    return next(error);
  }
...
const deletePlace = async (req, res, next) => {
  ...
  try {
    ...
    await Place.findByIdAndDelete(placeId, { session: currSession });

    place.creator.places.pull(place);
    await place.creator.save({ session: currSession });
